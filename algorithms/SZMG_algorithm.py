...
class SeismicMicrozonationAlgorithm(QgsProcessingAlgorithm):
    ...
    # processAlgorithm
    # =========================================================================
    def processAlgorithm(self, parameters: Dict[str, Any], context: Any, model_feedback: Any) -> Dict[str, Any]:
        ...
        try:
            # Step 1 -- Clip DTM
            feedback.pushInfo(self.tr('Step 1/6: Clipping DTM...'))
            ...
            outputs['slope'] = self._calculate_slope(parameters, outputs['clipped_dtm']['OUTPUT'], context, feedback)
            results[self.OUTPUT_SLOPE] = outputs['slope']['OUTPUT']
            feedback.setCurrentStep(2)

            # Step 3 -- Apply threshold
            feedback.pushInfo(self.tr('Step 3/6: Identifying high slopes...'))
            outputs['threshold_raster'] = self._apply_slope_threshold(parameters, outputs['slope']['OUTPUT'], context, feedback)
            feedback.setCurrentStep(3)

            # Step 4 -- Polygonize
            feedback.pushInfo(self.tr('Step 4/6: Converting to vector...'))
            outputs['polygons'] = self._polygonize_raster(outputs['threshold_raster']['OUTPUT'], context, feedback)
            feedback.setCurrentStep(4)

            # Step 5 -- Minimum area filter
            min_area = parameters.get(self.INPUT_MIN_AREA, 0.0)
            if min_area > 0:
                outputs['filtered'] = self._filter_by_min_area(outputs['polygons']['OUTPUT'], min_area, context, feedback)
            else:
                outputs['filtered'] = outputs['polygons']
            feedback.setCurrentStep(5)

            # Step 6 -- Join attributes
            feedback.pushInfo(self.tr('Step 6/6: Joining attributes...'))
            outputs['final'] = self._join_attributes(parameters, outputs['filtered']['OUTPUT'], context, feedback)
            results[self.OUTPUT_ZONES] = outputs['final']['OUTPUT']

            # --- SALVATAGGIO DATI PER POST-PROCESS ---
            # Salviamo gli ID/Percorsi per ritrovarli dopo il caricamento in mappa
            ...
            feedback.pushInfo(self.tr('Processing completato con successo!'))

        except Exception as e:
            raise QgsProcessingException(self.tr('Errore: {}').format(str(e)))

        return results

    # =========================================================================
    # postProcessAlgorithm -- Qui applichiamo gli stili
    # =========================================================================
    def postProcessAlgorithm(self, context: Any, feedback: Any) -> Dict[str, Any]:
        # Costruzione percorso dinamico: algorithms/.. -> root/styles/
        current_file_path = os.path.abspath(__file__)
        plugin_root = os.path.dirname(os.path.dirname(current_file_path))
        styles_dir = os.path.join(plugin_root, 'styles')

        # 1. Applica stile al VETTORE
        v_layer = QgsProcessingUtils.mapLayerFromString(self._output_zones_id, context)
        if v_layer:
            v_layer.setName(self.tr("Zone di Pendenza Critica"))
            qml_v = os.path.join(styles_dir, 'szmg_vector.qml')
            if os.path.exists(qml_v):
                v_layer.loadNamedStyle(qml_v)
                # Aggiorna le etichette in base alla soglia inserita dall'utente
                self._update_vector_labels(v_layer)
            else:
                feedback.reportError(f"File stile non trovato: {qml_v}")

        # 2. Applica stile al RASTER
        r_layer = QgsProcessingUtils.mapLayerFromString(self._output_slope_id, context)
        if r_layer:
            r_layer.setName(self.tr("Mappa delle Pendenze (Deg)"))
            qml_r = os.path.join(styles_dir, 'szmg_raster.qml')
            if os.path.exists(qml_r):
                r_layer.loadNamedStyle(qml_r)
            else:
                feedback.reportError(f"File stile non trovato: {qml_r}")

        return {}

    def _update_vector_labels(self, layer: QgsVectorLayer):
        """Aggiorna i testi della legenda nel pannello layer"""
        renderer = layer.renderer()
        if isinstance(renderer, QgsCategorizedSymbolRenderer):
            lbl_low = self.tr(f"Pendenza < {self._slope_threshold}° (Stabile)")
            lbl_high = self.tr(f"Pendenza >= {self._slope_threshold}° (Critica)")
            for cat in renderer.categories():
                if str(cat.value()) == '0': cat.setLabel(lbl_low)
                if str(cat.value()) == '1': cat.setLabel(lbl_high)
    ...
    def _apply_vector_style(
        ...
        qml_path = os.path.join(styles_dir, 'szmg_vector.qml')
        if not os.path.isfile(qml_path):
            self._log_warning(
                self.tr('[SZMG] szmg_vector.qml not found in {} -- vector symbology skipped.').format(styles_dir)
            )
            return
    ...
    def _apply_raster_style(
        ...
        qml_path = os.path.join(styles_dir, 'szmg_raster.qml')
        if not os.path.isfile(qml_path):
            self._log_warning(
                self.tr('[SZMG] szmg_raster.qml not found in {} -- raster symbology skipped.').format(styles_dir)
            )
            return
